...
namespace Guideon.Test
{
    ...
    public class MascotTestController : MonoBehaviour
    {
        ...
        private void DrawProceduralTab()
        {
            ...
            // 표정 오버레이 설정
            var overlay = _proceduralAnimator.ExpressionOverlay;
            if (overlay != null)
            {
                GUILayout.Label("<b>Expression Overlay:</b>", richStyle);
                GUILayout.Label("Overlay is active. Expression changes\nautomatically with state.", richStyle);
            }
            else
            {
                GUILayout.Label("<color=yellow>Expression Overlay: Head bone not found</color>", richStyle);
            }
